...
class HeteroGraphSAGE(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_node_types, type_embedding_dim, dropout_rate):
        ...
        self.relation_pred = Linear(2 * out_channels, len(edge_type_names))  # To be updated later based on number of edge types
...
# Update relation prediction layer according to number of edge types
...
def predict_relation(node_id_1, node_id_2):
    ...
    # Pass embeddings to relation prediction layer
...
test_files = [
    r'E:\Project\CM-GCN\Dataset\test(Security Task)\APT group and malware attribution.csv',
    r'E:\Project\CM-GCN\Dataset\test(Security Task)\APT group profiling.csv',
    r'E:\Project\CM-GCN\Dataset\test(Security Task)\Attack target prediction.csv',
    r'E:\Project\CM-GCN\Dataset\test(Security Task)\Attack technique simulation.csv',
    r'E:\Project\CM-GCN\Dataset\test(Security Task)\Malware propagation analysis.csv',
]
...
for file_name in test_files:
    ...
    try:
        ...
        # Get node pair IDs from first and second columns, and correct relation types from third column
