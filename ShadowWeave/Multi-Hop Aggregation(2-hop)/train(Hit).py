...
# Create a mapping dictionary from edge type to code for subsequent queries
...
# ==============================
# 2. Model Definition (modified to two-layer HeteroConv)
# ==============================
...
class HeteroGraphSAGE(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_node_types, type_embedding_dim, dropout_rate):
        ...
        # Define GraphSAGE convolution layers for heterogeneous graphs (simplified to two layers)
        self.conv1 = HeteroConv({
            ('node', 'to', 'node'): SAGEConv(in_channels + type_embedding_dim, hidden_channels)
        }, aggr='lstm')
        self.conv2 = HeteroConv({
            ('node', 'to', 'node'): SAGEConv(hidden_channels, out_channels)  # Directly from hidden to output
        }, aggr='lstm')
    ...
    def forward(self, x_dict, node_type_ids_dict, edge_index_dict):
        ...
        # Pass through heterogeneous GraphSAGE convolution layers (two layers)
        x_dict = self.conv1(x_dict, edge_index_dict)
        x_dict = {key: self.dropout(F.relu(x)) for key, x in x_dict.items()}
        x_dict = self.conv2(x_dict, edge_index_dict)  # Keep only two convolution layers
...
def calculate_hit_at_k(pred_scores, true_labels, k, edge_type_names=None, per_relation=False):
    ...
    # Check if true labels are among top k indices (per sample)
    hit = torch.any(topk_indices == true_labels.unsqueeze(1), dim=1).float()  # [num_edges], 1 indicates hit, 0 otherwise
