...
print('Number of relationship types:', len(edge_type_names))
print('Relationship type encodings:', edge_type_to_code)
...
# Used for storing training and validation losses and test metrics
train_losses = []
val_losses = []
test_metrics = []  # Stores Hit@1, Hit@3, Hit@10, MRR for each epoch
...
def calculate_hit_at_k(pred_scores, true_labels, k, edge_type_names=None, per_relation=False):
    """
    Calculate Hit@k metric (overall or per relationship type)
    """
    ...
    # Calculate Hit@k per relationship type
    hit_per_relation = {}
...
def calculate_mrr(pred_scores, true_labels, edge_type_names=None, per_relation=False):
    ...
    # Calculate MRR per relationship type
    mrr_per_relation = {}
    for rel_type in range(len(edge_type_names)):
        # Filter samples of current relationship type
        rel_mask = (true_labels == rel_type)
...
with torch.no_grad():
    ...
    # Evaluation metrics per relationship type (including MRR)
    ...
    final_mrr_per_rel = calculate_mrr(  # Calculate MRR per relationship type
    ...
    print("\nHit@1 per relationship type:")
    for rel, hit in final_hit1_per_rel.items():
        print(f'  Relationship {rel}: {hit * 100:.2f}%')
    ...
    for rel, mrr in final_mrr_per_rel.items():
        print(f'  Relationship {rel}: {mrr:.4f}')
    print("=" * 50 + "\n")
