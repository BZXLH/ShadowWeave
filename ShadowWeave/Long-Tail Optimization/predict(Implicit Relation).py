...
class HeteroGraphSAGE(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_node_types, type_embedding_dim, dropout_rate):
        ...
        self.relation_pred = Linear(2 * out_channels, len(edge_type_names))  # To be updated based on number of edge types later
...
def predict_relation(node_id_1, node_id_2):
    """
    Predict the relation type between two nodes.
    ...
    Returns:
        str: Predicted relation type name.
    """
...
node1 = test_df[0].values  # Data of the first column
node2 = test_df[1].values  # Data of the second column

# Initialize counts for correct predictions and total predictions for different cases
correct_uses = 0
correct_indicates = 0
...
# Calculate prediction accuracies for different cases
