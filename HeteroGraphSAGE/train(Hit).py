...
# For storing training and validation losses and test metrics
train_losses = []
val_losses = []
...
def calculate_hit_at_k(pred_scores, true_labels, k, edge_type_names=None, per_relation=False):
    ...
    Args:
        pred_scores: Model output prediction scores [num_edges, num_relations]
        true_labels: True relation type labels [num_edges]
        k: Top k prediction results
        edge_type_names: List of relation type names (only needed when per_relation=True)
        per_relation: Whether to calculate Hit@k for each relation type separately

    Returns:
        If per_relation=False: Overall Hit@k value (float)
        If per_relation=True: Dictionary with keys as relation type names and values as corresponding Hit@k
    """
    # Get indices of top k predictions (sorted by score in descending order)
...
def calculate_mrr(pred_scores, true_labels, edge_type_names=None, per_relation=False):
    ...
    Args:
        ...
        true_labels: True relation type labels [num_edges]
        edge_type_names: List of relation type names (only needed when per_relation=True)
        per_relation: Whether to calculate MRR for each relation type separately

    Returns:
        If per_relation=False: Overall MRR value (float)
        If per_relation=True: Dictionary with keys as relation type names and values as corresponding MRR
    """
    # Sort prediction scores for each sample and get ranks (from high to low)
    # Ranks start from 1, higher scores get better ranks
...
plt.tight_layout()
fig_path = os.path.join(result_dir, 'figure.png')  # Save image to result directory
plt.savefig(fig_path, dpi=500)  # Save as high-DPI PNG
plt.show()
# plt.show()