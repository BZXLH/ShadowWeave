...
import copy
import yaml
import matplotlib.pyplot as plt  # New: visualization library
...
# Create mapping dictionary from edge type to code
...
# Get input feature dimension and number of relation types
...
model = HeteroRGAT(
    ...
    heads=config['model'].get('heads', 8)  # Default to 1 head
)
print(model)
...
train_losses = []
val_losses = []
test_metrics = []  # Store Hit@1, Hit@3, Hit@10, MRR
...
for epoch in range(1, num_epochs + 1):
    ...
    # Early stopping check (based on Hit@1)
